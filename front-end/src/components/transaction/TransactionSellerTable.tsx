...
const statusColorMap = new Map<string, string>([
	["completed", "bg-green-300 text-green-800 hover:bg-green-400 hover:text-green-900"],
	["pending", "bg-yellow-300 text-black hover:bg-yellow-400"],
	["canceled", "bg-red-300 text-red-800 hover:bg-red-400 hover:text-red-900"],
	["processing", "bg-blue-300 text-blue-800 hover:bg-blue-400 hover:text-blue-900"],
]);
...
const getStatusColor = (status: string): string => {
	return (
		statusColorMap.get(status.toLowerCase()) || "bg-gray-300 text-gray-800 hover:bg-gray-400 hover:text-gray-900"
	);
};

const getStatusTooltipText = (status: string): string => {
	return statusTooltipMap.get(status.toLowerCase()) || "This transaction is currently being processed.";
};

const TransactionSellerTable: React.FC = () => {
	const [openEditDialog, setOpenEditDialog] = useState(false);
	const [transactionToEdit, setTransactionToEdit] = useState<number | null>(null);
	...
	const confirmEdit = () => {
		console.log(`Transaction ${transactionToEdit} status changed to ${newStatus}`);
		...
		<Card className="p-0 mb-5">
							...
							<TableCell>
								<TooltipProvider>
									<Tooltip>
										...
										<TooltipContent>
											<span>{getStatusTooltipText(transaction.status)} Click To Edit</span>
										</TooltipContent>
									</Tooltip>
								</TooltipProvider>
							</TableCell>
							<TableCell>{formatIntToIDR(transaction.total_price)}</TableCell>
							<TableCell>
								<ul>
			...
			<CardFooter>
				<div className="text-xs text-muted-foreground">
					Showing <strong>{transactions.length}</strong> transactions. {message}
				</div>
			</CardFooter>
			...
			<Dialog open={openEditDialog} onOpenChange={setOpenEditDialog}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Select a new status for transaction <strong>{transactionToEdit}</strong>.
						</DialogDescription>
					</DialogHeader>
					<Select>
